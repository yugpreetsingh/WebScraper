...
function App() {
  ...
  const fun = (e) => {
    ...
    if (val == "title") {
      ...
      axios
        ...
        .then((res) => {setRat(JSON.parse(JSON.stringify(res.data.data))); setLin({});setData({});})
        .catch((err) => {console.log(err)});
    } else {
      axios
  ...
  const func = (e)=>{
    axios.post('http://localhost:3002/search',{data:inp})
    .then((res)=>{
...
<div className="mt-2 ms-5 ">
  <input placeholder="Title Or Url" onKeyDown={keyDownHandler} onChange={(e)=>{setInp(e.target.value)}}/>
</div>
  <p className="fs-5 mt-2" style={{cursor:"pointer"}} onClick={func}>&#128269;</p>
        </div>
        ...
        <h2 className="p-2 border-success form-control" >{item.title}</h2>
      </div>
      ))):<></>}
          
          
           
            {(rat&& rat.length>0)?rat.map(((item) => (
        <div className="col-5 border-success form-control p-1 m-2" key={item._id} style={{maxWidth:"550px"}}>
        ...
        <p className="fw-semibold">{item.ratings} &#9734;</p>
            ...
            {(lin&& lin.length>0)?lin.map(((item) => (
        <div className="col-5" key={item._id}>
        <h2 className="p-2 border-success form-control" >{item.link}</h2>
      </div>
      ))):<></>}


{(search && search._id)?
        <div className="col-5 border-success form-control" key={search._id}>
        <h2 className="fs-4 " >Title: {search.title}</h2>
        <h2 className="fs-5 " >Price: {search.price}</h2>
        <h2 className="fs-5 " >Instock: {search.Instock}</h2>
